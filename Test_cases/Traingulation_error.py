...
if __name__=='__main__':
    ...
    if os.path.exists(validation_dataset_filepath):
        ...
        for i in range(nb_of_samples):
                ...
                while notfound:
                    ...
                    point_number,point_coordinates=get_extrapoints_target_length(polygon,0.5,algorithm='del2d')
    ...
    #    polygon_with_points=polygon_with_points.reshape(nb_of_edges+nb_of_points,2)
        ...
        real_ordered_qualities=Triangulation_with_points.order_quality_matrix(real_qualities,polygon,np.vstack([polygon,inner_points]),check_for_equal=False)
        initial_elements,sub_elements=Triangulation_with_points.triangulate(polygon,inner_points,real_ordered_qualities,recursive=True,plot_mesh=True)
...
#    #    
    #    initial_elements,sub_elements=Triangulation_with_points.triangulate(polygon,predicted_points,ordered_qualities_with_predicted_points,recursive=False,plot_mesh=False)
    #    total_elements=concat_element_list(initial_elements,sub_elements)
    ...
    #    print(" Triangulation error due to point prediction :", 100*(mesh_minimum_quality-mesh_minimum_quality_with_predicted_points)/mesh_minimum_quality)
    #    
    ...
    #    
        ...
        # Reshape polygon with points for NN3 use
        polygon_with_predicted_points=np.vstack([polygon,predicted_points])
        polygon_with_predicted_points_reshaped=np.insert(polygon_with_predicted_points,nb_of_edges,polygon_with_predicted_points[0],axis=0)
        
        polygon_with_predicted_points_tensor=Variable(torch.from_numpy(polygon_with_predicted_points_reshaped)).resize(1,1,polygon_with_predicted_points_reshaped.shape[0],polygon_with_predicted_points_reshaped.shape[1]).type(torch.FloatTensor)
        
        
        predicted_qualities=NN3(polygon_with_predicted_points_tensor)
        
        predicted_qualities=predicted_qualities.detach().numpy().reshape(nb_of_edges,nb_of_edges+nb_of_points)
        
        ordered_qualities_NN=Triangulation_with_points.order_quality_matrix(predicted_qualities,polygon,np.vstack([polygon,predicted_points]),check_for_equal=True)
    
    
        try:
            with time_limit(20):
                initial_elements,sub_elements=Triangulation_with_points.triangulate(polygon,predicted_points,ordered_qualities_NN,recursive=True,plot_mesh=True)
                total_elements=concat_element_list(initial_elements,sub_elements)
                mesh_minimum_quality_NN=compute_minimum_quality_triangulated_contour(polygon_with_predicted_points,total_elements)
                mesh_mean_quality_NN=compute_mean_quality_triangulated_contour(polygon_with_predicted_points,total_elements)
                ...
                triangularion_errors_mean.append(triangulation_mean_error)
    ...
    triangularion_errors_mean=np.array(triangularion_errors_mean)
    print("Triangulation error using NN scheme for {} edges polygons with  {} inner points is {} %".format(nb_of_edges,nb_of_points,triangulation_errors.mean()))
    print("Triangulation mean error using NN scheme for {} edges polygons with  {} inner points is {} %".format(nb_of_edges,nb_of_points,triangularion_errors_mean.mean()))
